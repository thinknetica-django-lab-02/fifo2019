...
def sending_html_mail_task(subject, text_content, html_content, from_email,
                           to_list):
    return sending_html_mail(subject, text_content, html_content, from_email,
                             to_list)
...
def sending_new_products():
    ...
    subject = "Новый товары за неделю!"
    text_content = ""
    html_content = ""
...
def user_signed_up_(sender, request, user, **kwargs):
    subject, from_email, to_list = f"Пользователь {user}", \
                                   'paveldudkov003@gmail.com', \
                                   [user.email]
    text_content = 'Благодарим Вас за интерес к нашему сайту!'
    html_content = '<p>Благодарим Вас за интерес к нашему сайту!</p>'
    sending_html_mail_task.delay(subject, text_content, html_content,
                                 from_email, to_list)
...
def send_new_product(sender, instance, created, **kwargs):
    if created:
        ...
        text_content = f"Появился новый товар {instance.title}. " \
                       f"Все подробности по " \
                       f"ссылке {instance.get_absolute_url()}"
        html_content = f'''
